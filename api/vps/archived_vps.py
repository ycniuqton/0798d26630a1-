...
def archived_vps(request):
    user = request.user
    filterable_fields = ['hostname', 'location', 'ip', 'status', 'user__username', 'user_id', '_deleted']
    ...
    # custom _delete
    if '_deleted' in filters:
        filters['_deleted'] = True if filters['_deleted'] == 'true' else False
